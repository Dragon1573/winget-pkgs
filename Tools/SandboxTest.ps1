...
# WinGet Settings
$script:WinGetProxy = $Proxy
if (!$script:WinGetProxy) {
    $script:DefaultProxy = [System.Net.WebRequest]::GetSystemWebProxy().GetProxy("https://www.google.com/")
    $script:WinGetProxy = $script:DefaultProxy.Scheme + '://' + $script:DefaultProxy.Host + ':' + $script:DefaultProxy.Port
}
if ($null -ne $script:WinGetProxy) {
    Write-Information -MessageData "Proxy configuration detected: $script:WinGetProxy"
}
...
"@
    ...
    $releasesAPIResponse = Invoke-RestMethod -Proxy $script:WinGetProxy @requestParameters
...
function Get-RemoteContent {
    ...
    $response = [String]::IsNullOrWhiteSpace($URL) ? @{StatusCode = 400 } : $(Invoke-WebRequest -Proxy $script:WinGetProxy -Uri $URL -Method Head -ErrorAction SilentlyContinue) # If the URL is null, return a status code of 400
...
function Test-GithubToken {
    ...
    $apiResponse = Invoke-WebRequest -Proxy $script:WinGetProxy @requestParameters # This will return an exception if the token is not valid; It is intentionally not caught
    ...
    $tokenExpiration = [DateTime]::Parse($tokenExpiration[0].ToString().TrimEnd("UTC")).ToLongDateString().Trim()
...
winget settings set DefaultProxy $script:WinGetProxy
