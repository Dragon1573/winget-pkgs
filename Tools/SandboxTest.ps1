...
"@
    ...
    $releasesAPIResponse = if ($Proxy) {Invoke-RestMethod -Proxy $Proxy @requestParameters} else { Invoke-RestMethod @requestParameters }
...
function Get-RemoteContent {
    ...
    if ([String]::IsNullOrWhiteSpace($URL)) {
        ...
        $response = if ($Proxy) { Invoke-WebRequest -Proxy $Proxy -Uri $URL -Method Head -ErrorAction SilentlyContinue } else { Invoke-WebRequest -Uri $URL -Method Head -ErrorAction SilentlyContinue }
    }
...
function Test-GithubToken {
    ...
    $apiResponse = if ($Proxy) { Invoke-WebRequest -Proxy $Proxy @requestParameters } else { Invoke-WebRequest @requestParameters } # This will return an exception if the token is not valid; It is intentionally not caught
