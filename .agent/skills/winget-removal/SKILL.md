...
1. Inspect package identifier of package name from user's prompt, as `<keyword>` here.
   ...
   - If you can't find that, fallback to normal fuzzy search without `--id` switch. There might by typo errors in user's input.
   - If searched package identifier **ISN'T EXACTLY MATCH** what was been inferred by user, **PAUSE** and ask for user's clarification.
2. Get all available versions under local folders
   - Manifests are stored with unified pattern `manifests/<prefix>/<package_identifier>/<version>`
     - A package identifier will looks like `Foo.Bar.Baz`
     - `<package_identifier>` is what was described above, but replace all dots `.` with slashes `/`
     - `<prefix>` is the very first letter of `<package_identifier>`, converted to lowercase
     - When there're multiple versions should be removed, only one version is accepted by the script. Run script multiple times with each version, to generate multiple branches for those removal.
3. Check which version(s) user want to remove.
   1. If he/she didn't explicitly mentioned, **PAUSE** here and ask for clear instructions.
      - **DO NOT ACT WITHOUT KNOWING EXPLICITLY VERSION RANGE!**
      - If built-in tools available, use those tools and provide a selectable lists for user to claim.
      - If no such tool, **PAUSE** and ask user for clear instructions.
   2. If user provide a fuzzy natural language "version range", try map it to exact array of versions.
      - After guessing, **PAUSE**, explain your guess to users and then get approval or rejection from user.
4. Invoke `Remove-Package.ps1` for actual removal.
   ...
   - Pass `-IssueId` integer to link the pull request to a known issue.
     - If user didn't provide this information, try invoke built-in tools asking user to claim
     - If no such tools available, **STOP** and express here're missing information your need. Ask user to provide it in the next chat message.
   - Pass `-Assistant` with AI agent specific attribution.
   - Pass `-AutoPush` if user agree to automatically push branches.
   - Pass `-AutoSubmit` if user agree to automatically create pull requests on GitHub.
5. If `-AutoPush` is appended, ask if user want to remove those generated local branches after step 4.
